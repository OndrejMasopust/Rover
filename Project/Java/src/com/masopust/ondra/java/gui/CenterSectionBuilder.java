...
public class CenterSectionBuilder {
	...
	public static void build(Scene scene, Pane centerSection, int numberOfLines, Rectangle2D screenBounds, ArrayList<Line> lines, ArrayList<Rectangle> dots) {
		int lineLength = 200; //FIXME
		double startX = screenBounds.getWidth() / 2;
		double startY = screenBounds.getHeight() / 2;
		...
		for (int i = 0; i < numberOfLines; i++) {
			...
			//set start and end points to be relative to the scene dimensions
			lines.get(i).startXProperty().bind(scene.widthProperty().divide(2));
			lines.get(i).startYProperty().bind(scene.heightProperty().divide(2));
